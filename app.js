...
// Função para calcular todos os períodos desde a data inicial
...
// ========================================
// 🎨 SISTEMA DE MODAIS BONITOS
// ========================================

// Função para mostrar modal de notificação (substitui alert)
function showNotificationModal(title, message, icon = 'ℹ️') {
...
function showWarningModal(message) {
    return showNotificationModal('Atenção', message, '⚠️');
}

// Função para mostrar modal de informação
function showInfoModal(message) {
    return showNotificationModal('Informação', message, 'ℹ️');
}

// ========================================
// 🔄 SUBSTITUIÇÕES DOS ALERTS E CONFIRMS
// ========================================

// Substituir alert() por showNotificationModal()
function alert(message) {
    return showNotificationModal('Notificação', message, 'ℹ️');
}

// Substituir confirm() por showConfirmationModal()
function confirm(message) {
    return showConfirmationModal('Confirmar', message, '❓');
}
...
let maxCacheSize = 20; // Reduzido para apenas 20 transações (uma página)
let cacheCleanupTimer = null; // Timer para limpeza automática
...
function setupEventListeners() {
    ...
    closeTransactionsModal.addEventListener('click', () => {
        ...
        // Limpar cache quando fechar modal de transações
    ...
    // Botão para deletar todas as transações
    document.getElementById('delete-all-transactions-btn').addEventListener('click', () => {
        showConfirmModal(
            'Deletar Todas as Transações', 
            '⚠️ ATENÇÃO: Esta ação irá deletar TODAS as suas transações e resetar os saldos das contas para zero. Esta ação não pode ser desfeita e afetará apenas os seus dados (não da sua esposa).\n\nTem certeza que deseja continuar?', 
            () => deleteAllTransactions()
        );
    });
}
...
// Configurar listeners do Firestore com otimização para milhões de transações
function setupFirestoreListeners() {
    ...
    transactionsUnsubscribe = onTransactionsUpdate((transactionsData) => {
        // Otimização: Manter apenas as últimas 50 transações em memória para o listener
        // O sistema de paginação carregará mais conforme necessário
        transactions = transactionsData.slice(0, 50);
        
        console.log('💾 Transações recentes carregadas:', transactions.length);
        
        updateFinancialSummary();
        renderRecentTransactions();
        
        // Atualizar status das contas fixas quando transações mudam
...
// Função para atualizar status das contas fixas
async function updateFixedBillsStatus() {
    try {
        ...
        // Verificar se alguma conta precisa de expansão automática (processamento no servidor)
        ...
        for (const bill of fixedBills) {
            ...
            // Usar o lastPaidPeriod já calculado pela função getFixedBillStatus (processamento no servidor)
            if (bill.lastPaidPeriod) {
                ...
                // Se não tem pagamento ainda, verificar se precisa expandir
                const initialDate = bill.initialDate?.toDate ? bill.initialDate.toDate() : new Date(bill.initialDate);
                const totalPeriods = bill.totalPeriodsGenerated || 12;
                const periods = calculateAllPeriodsFromInitial(initialDate, bill.dueDay, totalPeriods, false);
                
                // Se tem poucos períodos restantes, precisa expandir
                if (periods.length <= 5) {
                    needsExpansion = true;
                    break;
                }
...
// Processar resultado do Gemini e executar ações apropriadas
async function processGeminiResult(result, originalMessage) {
    try {
        // Mostrar resposta do Gemini APENAS se não for uma ação que precisa de confirmação
        ...
        switch (result.intent) {
            ...
            case 'GREETING':
            case 'HELP':
                // Para saudações, mostrar apenas a resposta do Gemini
                if (result.response) {
                    addChatMessage('assistant', result.response);
                }
                break;
            default:
                console.warn('Intenção não reconhecida:', result.intent);
                // Só mostrar mensagem de erro se não tiver resposta do Gemini
                if (!result.response) {
                    addChatMessage('assistant', 'Desculpe, não entendi sua solicitação. Pode reformular?');
                }
        }
    } catch (error) {
        console.error('Erro ao processar resultado do Gemini:', error);
        addChatMessage('assistant', '❌ Ocorreu um erro ao processar sua solicitação. Tente novamente.');
    }
}
...
async function handleDatabaseQuery(queryType, originalMessage) {
    try {
        ...
        addChatMessage('assistant', '❌ Erro ao consultar dados. Tente novamente.');
    }
}
...
async function handleIncomeByDate(originalMessage) {
    ...
    if (yesterdayIncome.length === 0) {
        addChatMessage('assistant', '📊 Você não teve receitas ontem.');
        return;
    }
    
    const totalIncome = yesterdayIncome.reduce((sum, t) => sum + t.amount, 0);
    
    let response = `📊 **Receitas de ontem (${yesterday.toLocaleDateString('pt-BR')}):**\n\n`;
    response += `💰 **Total:** R$ ${totalIncome.toFixed(2)}\n\n`;
...
// Handler para transações por período
async function handleTransactionsByPeriod(originalMessage) {
    ...
    if (monthlyTransactions.length === 0) {
        addChatMessage('assistant', '📊 Você não tem transações este mês.');
        return;
    }
    ...
    let response = `📊 **Resumo do mês (${today.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}):**\n\n`;
    response += `💰 **Receitas:** R$ ${totalIncome.toFixed(2)}\n`;
    response += `💸 **Despesas:** R$ ${totalExpenses.toFixed(2)}\n`;
    response += `📈 **Saldo:** R$ ${balance.toFixed(2)}\n\n`;
    response += `📋 **Total de transações:** ${monthlyTransactions.length}`;
...
// Handler para contas fixas não pagas
async function handleUnpaidBills(originalMessage) {
    if (fixedBills.length === 0) {
        addChatMessage('assistant', '📋 Você não tem contas fixas cadastradas.');
        return;
    }
    ...
    for (const bill of fixedBills) {
        // Verificar se a conta fixa já foi paga este mês
    ...
    if (unpaidBills.length === 0) {
        addChatMessage('assistant', '✅ Todas as suas contas fixas estão em dia!');
        return;
    }
    
    let response = `📋 **Contas fixas não pagas este mês:**\n\n`;
...
async function handleAddExpense(entities, originalMessage) {
    if (!entities.amount || entities.amount <= 0) {
        addChatMessage('assistant', 'Por favor, informe um valor válido para a despesa.');
        return;
    }
    ...
    if (!entities.fromAccount) {
        addChatMessage('assistant', `Registrei sua despesa de R$ ${entities.amount.toFixed(2)} - ${description}. De qual conta você quer debitar?`, [
    ...
    if (!account) {
        addChatMessage('assistant', `❌ Conta "${entities.fromAccount}" não encontrada. Contas disponíveis: ${accounts.map(a => a.name).join(', ')}`);
        return;
    }
    
    // Verificar saldo suficiente
    if (account.balance < entities.amount) {
        addChatMessage('assistant', `❌ Saldo insuficiente na conta ${account.name}. Saldo atual: R$ ${account.balance.toFixed(2)}`);
        return;
    }
...
async function handleAddIncome(entities, originalMessage) {
    if (!entities.amount || entities.amount <= 0) {
        addChatMessage('assistant', 'Por favor, informe um valor válido para a receita.');
        return;
    }
    ...
    if (!entities.toAccount) {
        addChatMessage('assistant', `Registrei sua receita de R$ ${entities.amount.toFixed(2)} - ${description}. Em qual conta você quer creditar?`, [
    ...
    if (!account) {
        addChatMessage('assistant', `❌ Conta "${entities.toAccount}" não encontrada. Contas disponíveis: ${accounts.map(a => a.name).join(', ')}`);
        return;
    }
...
async function handleTransfer(entities, originalMessage) {
    if (!entities.amount || entities.amount <= 0) {
        addChatMessage('assistant', 'Por favor, informe um valor válido para a transferência.');
        return;
    }
    
    if (!entities.fromAccount || !entities.toAccount) {
        addChatMessage('assistant', 'Por favor, especifique as contas de origem e destino para a transferência.');
        return;
    }
    ...
    if (!fromAccount) {
        addChatMessage('assistant', `❌ Conta de origem "${entities.fromAccount}" não encontrada. Contas disponíveis: ${accounts.map(a => a.name).join(', ')}`);
        return;
    }
    
    if (!toAccount) {
        addChatMessage('assistant', `❌ Conta de destino "${entities.toAccount}" não encontrada. Contas disponíveis: ${accounts.map(a => a.name).join(', ')}`);
        return;
    }
    
    if (fromAccount.id === toAccount.id) {
        addChatMessage('assistant', '❌ Não é possível transferir para a mesma conta.');
        return;
    }
    
    // Verificar saldo suficiente na conta de origem
    if (fromAccount.balance < entities.amount) {
        addChatMessage('assistant', `❌ Saldo insuficiente na conta ${fromAccount.name}. Saldo atual: R$ ${fromAccount.balance.toFixed(2)}`);
        return;
    }
    
    // Executar transferência
    await performTransfer(fromAccount.id, toAccount.id, entities.amount, 'Transferência entre contas');
    
    addChatMessage('assistant', `✅ Transferência de R$ ${entities.amount.toFixed(2)} realizada de ${fromAccount.name} para ${toAccount.name}`);
}

// Handler para pagamento de conta fixa
async function handlePayFixedBill(entities, originalMessage) {
    if (!entities.fixedBillName) {
        addChatMessage('assistant', 'Por favor, especifique qual conta fixa você pagou.');
        return;
    }
...
async function handleQueryBalance(entities, originalMessage) {
    if (entities.fromAccount) {
        ...
        if (account) {
            addChatMessage('assistant', `💰 **Saldo da conta ${account.name}:** R$ ${account.balance.toFixed(2)}`);
        } else {
            addChatMessage('assistant', `❌ Conta "${entities.fromAccount}" não encontrada. Contas disponíveis: ${accounts.map(a => a.name).join(', ')}`);
        }
    } else {
        ...
        addChatMessage('assistant', `💰 **Seu saldo total:** R$ ${totalBalance.toFixed(2)}`);
...
// Handler para consulta de transações
async function handleQueryTransactions(entities, originalMessage) {
    // Se não especificou tipo de consulta, mostrar transações recentes
    const recentTransactions = transactions.slice(0, 10);
    
    if (recentTransactions.length === 0) {
        addChatMessage('assistant', '📋 Você ainda não tem transações registradas.');
        return;
    }
    
    let response = `📋 **Suas últimas ${recentTransactions.length} transações:**\n\n`;
    
    recentTransactions.forEach(t => {
        const icon = t.type === 'expense' ? '💸' : t.type === 'income' ? '💰' : '🔄';
        const date = new Date(t.date).toLocaleDateString('pt-BR');
        response += `${icon} **${t.description}**\n`;
        response += `   💰 R$ ${t.amount.toFixed(2)} - ${date}\n\n`;
    });
    
    addChatMessage('assistant', response);
}

// Handler para adicionar dívida
async function handleAddDebt(entities, originalMessage) {
    if (!entities.amount || entities.amount <= 0) {
        addChatMessage('assistant', 'Por favor, informe um valor válido para a dívida.');
        return;
    }
    
    const description = entities.description || 'Dívida';
    
    await addDebt({
        amount: entities.amount,
        description: description,
        creditor: entities.toAccount || 'Não especificado',
        date: new Date()
    });
    
    addChatMessage('assistant', `📝 Dívida de R$ ${entities.amount.toFixed(2)} registrada - ${description}`);
}
...
async function handleSendMessage() {
    ...
    try {
        // Validação robusta dos dados antes do processamento
        if (!currentUser) {
            throw new Error('Usuário não autenticado');
        }
        
        if (!accounts || !Array.isArray(accounts)) {
            console.warn('accounts inválido:', accounts);
            accounts = [];
        }
        
        if (!fixedBills || !Array.isArray(fixedBills)) {
            console.warn('fixedBills inválido:', fixedBills);
            fixedBills = [];
        }
        ...
        if (!result || typeof result !== 'object') {
            throw new Error('Resultado inválido do processamento de mensagem');
        }
        ...
        // Mensagem de erro mais amigável e inteligente
        ...
        if (error.message.includes('Valor deve ser maior que zero')) {
            ...
            errorMessage += 'Conta não encontrada. Verifique se a conta existe.';
        } else if (error.message.includes('Usuário não autenticado')) {
            errorMessage += 'Sessão expirada. Faça login novamente.';
        } else if (error.message.includes('Missing or insufficient permissions')) {
            errorMessage += 'Problema de permissões no banco de dados. Tente recarregar a página.';
        } else if (error.message.includes('Resultado inválido')) {
            errorMessage += 'Erro interno do sistema. Tente reformular sua mensagem.';
        } else {
            errorMessage += 'Tente reformular sua mensagem ou verificar se todos os dados estão corretos.';
        }
...
async function executeAction(payload) {
    try {
        ...
        if (!payload.action || typeof payload.action !== 'string') {
            throw new Error('Ação inválida no payload');
        }
        
        if (!payload.data || typeof payload.data !== 'object') {
            throw new Error('Dados inválidos no payload');
        }
        
        switch (payload.action) {
            case 'ADD_TRANSACTION':
                // A data já vem como objeto Date do NLU, não precisa converter
                ...
                const emoji = payload.data.type === 'expense' ? '💸' : '💰';
                ...
                let message = `${emoji} **${actionType.charAt(0).toUpperCase() + actionType.slice(1)} registrado com sucesso!**\n\n` +
                    `💰 **Valor:** R$ ${payload.data.amount.toFixed(2)}\n` +
                    `📝 **Descrição:** ${payload.data.description}\n` +
                    `🏦 **Conta:** ${account?.name || 'N/A'}\n` +
                    `📅 **Data:** ${payload.data.date ? new Date(payload.data.date).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR')}`;
            ...
            case 'PERFORM_TRANSFER':
                ...
                addChatMessage('assistant', 
                    ...
                    `💰 **Valor:** R$ ${payload.data.amount.toFixed(2)}\n` +
                    `📤 **De:** ${fromAccount?.name || 'N/A'}\n` +
                    `📥 **Para:** ${toAccount?.name || 'N/A'}\n` +
                    `📅 **Data:** ${new Date().toLocaleDateString('pt-BR')}`
                );
                break;
            ...
            case 'ADD_DEBT':
                ...
                addChatMessage('assistant', 
                    ...
                    `💰 **Valor:** R$ ${payload.data.amount.toFixed(2)}\n` +
                    `👤 **Para:** ${payload.data.debtorName}\n` +
                    `📝 **Descrição:** ${payload.data.description}\n` +
                    `📅 **Data:** ${new Date().toLocaleDateString('pt-BR')}\n\n` +
                    `💡 **Dica:** Use "minhas dívidas" para consultar todas as suas dívidas pendentes.`
                );
                break;
            ...
            default:
                addChatMessage('assistant', '🤔 Ação não implementada ainda. Estou aprendendo novas funcionalidades!');
        }
    } catch (error) {
        console.error('Erro ao executar ação:', error);
        
        // Mensagem de erro mais específica e inteligente
        ...
        if (error.message.includes('Valor deve ser maior que zero')) {
            ...
            errorMessage += 'Problema de permissões no banco de dados. Atualize as regras do Firestore ou recarregue a página.';
        } else if (error.message.includes('Payload inválido') || error.message.includes('Ação inválida') || error.message.includes('Dados inválidos')) {
            errorMessage += 'Erro interno do sistema. Tente reformular sua mensagem.';
        } else {
...
// Gerar resumo financeiro avançado
function generateAdvancedSummary(data) {
    ...
    let message = `📊 **Resumo Financeiro do Mês**\n\n`;
    message += `💰 **Receitas:** R$ ${income.toFixed(2)}\n`;
    message += `💸 **Despesas:** R$ ${expenses.toFixed(2)}\n`;
    message += `💵 **Saldo do Mês:** R$ ${balance.toFixed(2)}\n`;
    ...
    if (balance > 0) {
        message += `✅ **Análise:** Você está com saldo positivo este mês! Parabéns!`;
    } else if (balance < 0) {
        message += `⚠️ **Análise:** Você está com saldo negativo. Considere revisar seus gastos.`;
    } else {
        message += `📊 **Análise:** Seus gastos estão equilibrados com suas receitas.`;
    }
...
function generateAnalytics(data) {
    ...
    let message = `📈 **Análise Detalhada dos Gastos**\n\n`;
    
    if (topCategories.length > 0) {
        message += `🏆 **Top 5 Categorias de Gastos:**\n`;
        topCategories.forEach(([category, stats], index) => {
            ...
            message += `   💰 Total: R$ ${stats.total.toFixed(2)}\n`;
            message += `   📊 Média: R$ ${stats.avg.toFixed(2)}\n`;
            message += `   🔢 Transações: ${stats.count}\n`;
    ...
    if (income.length > 0) {
        ...
        message += `💰 **Análise de Receitas:**\n`;
        message += `📊 Total: R$ ${totalIncome.toFixed(2)}\n`;
        message += `📈 Média por transação: R$ ${avgIncome.toFixed(2)}\n`;
        message += `🔢 Número de receitas: ${income.length}\n\n`;
    }
    ...
    if (topCategories.length > 0) {
        ...
        message += `• Sua maior categoria de gastos é **${biggestCategory[0]}** (${((biggestCategory[1].total / expenses.reduce((sum, t) => sum + t.amount, 0)) * 100).toFixed(1)}%)\n`;
    }
    ...
    if (totalIncome > 0) {
        ...
        message += `• Taxa de poupança: ${savingsRate}%\n`;
    }
    
    if (expenses.length > 0) {
        const avgExpense = totalExpenses / expenses.length;
        message += `• Gasto médio por transação: R$ ${avgExpense.toFixed(2)}\n`;
    }
    
    return message;
}

// Gerar relatório de gastos diários
function generateDailyExpenses(data) {
    ...
    let message = `💸 **Gastos ${periodText.charAt(0).toUpperCase() + periodText.slice(1)}**\n\n`;
    
    if (expenses.length === 0) {
        message += `✅ **Nenhum gasto registrado ${periodText}!**\n\n`;
        message += `🎉 Parabéns! Você não gastou nada ${periodText}.`;
        return message;
    }
    
    message += `💰 **Total gasto:** R$ ${totalExpenses.toFixed(2)}\n`;
    message += `📊 **Número de gastos:** ${expenses.length}\n\n`;
    ...
    Object.entries(categoryGroups).forEach(([category, categoryExpenses]) => {
        ...
        message += `🏷️ **${category.charAt(0).toUpperCase() + category.slice(1)}**\n`;
        message += `   💰 Total: R$ ${categoryTotal.toFixed(2)} (${percentage}%)\n`;
        message += `   🔢 Transações: ${categoryExpenses.length}\n`;
        
        // Listar transações individuais
        categoryExpenses.forEach(expense => {
            const time = expense.date && typeof expense.date.toDate === 'function' 
                ...
                : 'N/A';
            message += `   • R$ ${expense.amount.toFixed(2)} - ${expense.description} (${time})\n`;
        });
        message += '\n';
    });
    ...
    if (totalExpenses > 0) {
        ...
        if (expenses.length === 1) {
            message += `• Você fez apenas 1 gasto ${periodText}\n`;
        } else {
            message += `• Você fez ${expenses.length} gastos ${periodText}\n`;
        }
    }
...
// Gerar relatório de receitas diárias
function generateDailyIncome(data) {
    ...
    let message = `💰 **Receitas ${periodText.charAt(0).toUpperCase() + periodText.slice(1)}**\n\n`;
    
    if (income.length === 0) {
        message += `📉 **Nenhuma receita registrada ${periodText}!**\n\n`;
        message += `💡 Você não recebeu nenhum dinheiro ${periodText}.`;
        return message;
    }
    
    message += `💵 **Total recebido:** R$ ${totalIncome.toFixed(2)}\n`;
    message += `📊 **Número de receitas:** ${income.length}\n\n`;
    ...
    Object.entries(categoryGroups).forEach(([category, categoryIncome]) => {
        ...
        message += `🏷️ **${category.charAt(0).toUpperCase() + category.slice(1)}**\n`;
        message += `   💰 Total: R$ ${categoryTotal.toFixed(2)} (${percentage}%)\n`;
        message += `   🔢 Transações: ${categoryIncome.length}\n`;
        ...
        categoryIncome.forEach(incomeItem => {
            const time = incomeItem.date && typeof incomeItem.date.toDate === 'function' 
                ...
                : 'N/A';
            message += `   • R$ ${incomeItem.amount.toFixed(2)} - ${incomeItem.description} (${time})\n`;
        });
        message += '\n';
    });
    ...
    if (totalIncome > 0) {
        const avgIncome = totalIncome / income.length;
        message += `📈 **Análise:**\n`;
        message += `• Receita média por transação: R$ ${avgIncome.toFixed(2)}\n`;
        
        if (income.length === 1) {
            message += `• Você recebeu apenas 1 entrada ${periodText}\n`;
        } else {
            message += `• Você recebeu ${income.length} entradas ${periodText}\n`;
        }
    }
...
// Gerar relatório de transferências diárias
function generateDailyTransfers(data) {
    ...
    let message = `🔄 **Transferências ${periodText.charAt(0).toUpperCase() + periodText.slice(1)}**\n\n`;
    
    if (transfers.length === 0) {
        message += `📭 **Nenhuma transferência realizada ${periodText}!**\n\n`;
        message += `💡 Você não fez nenhuma transferência entre suas contas ${periodText}.`;
        return message;
    }
    
    message += `📊 **Número de transferências:** ${transfers.length}\n\n`;
    
    // Listar transferências
    message += `📋 **Detalhamento das transferências:**\n\n`;
    ...
    transfers.forEach((transfer, index) => {
        ...
        message += `   📤 **De:** ${fromAccount?.name || 'Conta não encontrada'}\n`;
        message += `   📥 **Para:** ${toAccount?.name || 'Conta não encontrada'}\n`;
        message += `   🕐 **Horário:** ${time}\n`;
        if (transfer.description) {
            message += `   📝 **Descrição:** ${transfer.description}\n`;
        }
        message += '\n';
    });
    ...
    if (transfers.length > 0) {
        ...
        message += `📈 **Análise:**\n`;
        message += `• Valor total transferido: R$ ${totalTransferred.toFixed(2)}\n`;
        message += `• Valor médio por transferência: R$ ${avgTransfer.toFixed(2)}\n`;
        
        if (transfers.length === 1) {
            message += `• Você fez apenas 1 transferência ${periodText}\n`;
        } else {
            message += `• Você fez ${transfers.length} transferências ${periodText}\n`;
        }
    }
...
function renderFixedBills() {
    ...
    topBills.forEach(bill => {
        ...
        // Usar status calculado pelo Firebase se disponível, senão calcular localmente
        let status, statusClass, statusText, totalPaid, remainingAmount;
        
        if (bill.status) {
            // Status já calculado pelo Firebase
            status = bill.status;
        ...
        billElement.innerHTML = `
            ...
            <div class="flex justify-between items-center text-sm">
                ...
                ${remainingAmount > 0 ? `<span class="${statusClass}">${statusText}</span>` : `<span class="text-green-400 bg-green-900 bg-opacity-30 px-2 py-1 rounded-full text-xs">✓ Paga</span>`}
            </div>
        `;
...
function renderBillsView() {
    ...
    sortedBills.forEach(bill => {
        ...
        if (bill.status) {
            ...
            totalPaid = bill.totalPaid || 0;
            remainingAmount = bill.remainingAmount || 0;
            canPay = bill.canPay !== false; // Sempre true quando há valor restante
        } else {
            // Calcular status localmente (fallback)
            status = calculateBillStatus(bill);
            statusClass = getStatusClass(status);
            ...
            totalPaid = billTransactions.reduce((sum, t) => sum + t.amount, 0);
            remainingAmount = bill.amount - totalPaid;
            canPay = true; // Sempre pode pagar quando há valor restante
        }
        
        // Determinar texto do botão de pagamento
        ...
        if (remainingAmount > 0) {
            ...
            paymentButtonText = 'Pagar Próximo';
        }
        
        // Obter último período pago para mostrar
        const lastPaidPeriod = bill.lastPaidPeriod;
        const lastPaidText = lastPaidPeriod ? 
            `Último pago: ${lastPaidPeriod.monthName} ${lastPaidPeriod.year}` : 
            'Nenhum pagamento';
...
// Função para agrupar transferências e evitar duplicação no histórico
function groupTransferTransactions(transactions) {
    const grouped = [];
    const processedTransferIds = new Set();
    
    transactions.forEach(transaction => {
        // Se é uma transferência que já foi processada, pular
        if (transaction.transferId && processedTransferIds.has(transaction.transferId)) {
            return;
        }
        
        // Se é uma transferência de entrada (transfer_in), criar uma entrada única
        if (transaction.type === 'transfer_in') {
            processedTransferIds.add(transaction.transferId);
            
            // Encontrar a transação de saída correspondente
            const outTransaction = transactions.find(t => 
                t.transferId === transaction.transferId && t.type === 'transfer_out'
            );
            
            if (outTransaction) {
                // Criar uma entrada única para a transferência
                grouped.push({
                    ...transaction,
                    description: `Transferência de ${outTransaction.accountName} para ${transaction.accountName}`,
                    type: 'transfer',
                    amount: transaction.amount
                });
            } else {
                // Se não encontrar a transação de saída, usar a entrada
                grouped.push(transaction);
            }
        }
        // Se é uma transferência de saída (transfer_out), pular (já foi processada acima)
        else if (transaction.type === 'transfer_out') {
            // Não adicionar, pois será processada junto com a entrada
            return;
        }
        // Para outros tipos de transação, adicionar normalmente
        else {
            grouped.push(transaction);
        }
    });
...
function renderRecentTransactions() {
    console.log('🎨 Renderizando transações recentes...');
    
    if (!recentTransactions) {
        console.error('❌ Elemento recentTransactions não encontrado!');
        return;
    }
    ...
    if (!transactions || transactions.length === 0) {
        ...
        recentTransactions.innerHTML = '<p class="text-gray-400 text-sm">Nenhuma transação recente</p>';
        return;
    }
    
    // Agrupar transferências para evitar duplicação
    const groupedTransactions = groupTransferTransactions(transactions);
    const recent = groupedTransactions.slice(0, 5);
    console.log('📄 Transações recentes para exibir:', recent.length);
    
    recent.forEach((transaction, index) => {
        console.log(`💳 Renderizando transação ${index + 1}:`, {
            id: transaction.id,
        ...
        const html = `
            <div class="flex justify-between items-center">
                <span class="text-gray-300 truncate">${transaction.description || 'Sem descrição'}</span>
                <span class="${amountClass} font-medium">${amountPrefix}R$ ${amount.toFixed(2)}</span>
            </div>
            <div class="text-xs text-gray-400">${transaction.accountName || 'Conta não especificada'} • ${formatDate(transaction.date)}</div>
        `;
        
        console.log(`📝 HTML gerado para transação ${index + 1}:`, html);
...
function renderTransactions() {
    console.log('🎨 Renderizando transações no modal...');
    
    transactionsList.innerHTML = '';
    
    if (!filteredTransactions || filteredTransactions.length === 0) {
        console.log('⚠️ Nenhuma transação filtrada encontrada');
        transactionsList.innerHTML = '<p class="text-gray-400 text-center py-8">Nenhuma transação encontrada</p>';
        return;
    }
    ...
    console.log('📄 Transações da página atual:', pageTransactions.length);
    ...
    pageTransactions.forEach((transaction, index) => {
        console.log(`💳 Renderizando transação ${index + 1} no modal:`, {
            id: transaction.id,
            description: transaction.description,
            amount: transaction.amount,
            type: transaction.type
        ...
        transactionElement.innerHTML = `
            <div class="flex-1">
                <div class="text-gray-200 font-medium">${transaction.description || 'Sem descrição'}</div>
                <div class="text-sm text-gray-400">${transaction.accountName || 'Conta não especificada'} • ${formatDate(transaction.date)}</div>
            </div>
            <div class="text-right">
...
// Função otimizada para renderizar transações com scroll virtual
function renderTransactionsOptimized() {
    console.log('🎨 Renderizando transações otimizadas...');
    ...
    // Renderizar apenas as transações visíveis (primeira página)
    // Agrupar transferências para evitar duplicação
    const groupedFilteredTransactions = groupTransferTransactions(filteredTransactions);
    ...
    console.log(`📄 Renderizando ${pageTransactions.length} transações (página ${currentPage})`);
...
function clearTransactionCache() {
    console.log('🧹 Limpando cache de transações...');
...
// Função para agendar limpeza automática do cache
function scheduleCacheCleanup() {
    // Limpar cache após 5 minutos de inatividade
    if (cacheCleanupTimer) {
        clearTimeout(cacheCleanupTimer);
    }
    
    cacheCleanupTimer = setTimeout(() => {
        console.log('⏰ Limpeza automática do cache executada');
        clearTransactionCache();
    }, 5 * 60 * 1000); // 5 minutos
}

// Função para inicializar o sistema otimizado de transações
async function initializeOptimizedTransactions() {
    try {
        ...
        console.log('✅ Sistema otimizado de transações inicializado');
    } catch (error) {
        console.error('Erro ao inicializar transações otimizadas:', error);
        showErrorModal('Erro ao carregar transações. Tente novamente.');
    }
}
...
// Função otimizada para carregar transações com paginação inteligente
async function loadMoreTransactionsInternal() {
    ...
    try {
        ...
        loadingElement.innerHTML = '🔄 Carregando mais transações...';
        transactionsList.appendChild(loadingElement);
        
        // Carregar transações usando cursor-based pagination
        ...
        if (result.transactions.length > 0) {
            ...
            if (allLoadedTransactions.length === 0) {
                // Primeira carga: adicionar transações
                allLoadedTransactions = [...result.transactions];
            } else {
                // Carregamentos subsequentes: substituir por novas transações
                allLoadedTransactions = [...result.transactions];
            }
            
            // Agendar limpeza automática após cada carregamento
            scheduleCacheCleanup();
            ...
            console.log(`📄 Carregadas ${result.transactions.length} transações. Total em cache: ${allLoadedTransactions.length}`);
        } else {
            hasMoreTransactions = false;
            console.log('📄 Não há mais transações para carregar');
        }
        ...
        showErrorModal('Erro ao carregar mais transações. Tente novamente.');
    } finally {
        isLoadingMore = false;
    }
}

// Função otimizada para filtrar transações
// Função para aplicar filtros atuais
...
// Função auxiliar para verificar se uma transação passa nos filtros atuais
...
// Função otimizada para filtrar transações (compatibilidade)
...
// Funções de pagamento de contas fixas
function handleFullPaymentModal(bill) {
    currentBillForPayment = bill;
    
    // Obter informações completas da conta fixa para permitir pagamento antecipado
    getFixedBillInfoOptimized(bill.id).then(billInfo => {
        // Verificar se há períodos disponíveis para pagamento
        const availablePeriods = billInfo.nextPeriodsToPay;
        
        if (availablePeriods.length === 0) {
            showWarningModal('Não há períodos disponíveis para pagamento');
            return;
        }
        
        // CORREÇÃO: Identificar o período atual (primeiro período com valor restante)
        let currentPeriod = availablePeriods.find(period => period.isCurrentMonth && period.remainingAmount > 0);
        
        // Se não há período atual com valor restante, verificar se há pagamento parcial em período futuro
        if (!currentPeriod) {
            // Buscar próximo período com pagamento parcial (que deve ser tratado como período atual)
        ...
        if (currentPeriod) {
            // PAGAMENTO NORMAL: Há período atual com valor restante - pagar diretamente
            ...
            // PAGAMENTO ANTECIPADO: Não há período atual com valor restante - mostrar modal de seleção
            // Filtrar apenas períodos futuros (não o atual)
            const futurePeriods = availablePeriods.filter(period => !period.isCurrentMonth && period.remainingAmount > 0);
            
            if (futurePeriods.length === 0) {
                showInfoModal('Não há períodos futuros disponíveis para pagamento antecipado');
                return;
            }
...
// Função para mostrar modal de seleção de períodos
            ...
            <div class="bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button id="close-period-selection-modal" class="text-gray-400 hover:text-white text-2xl">×</button>
                    </div>
                    
                    <div class="mb-4">
                        <h4 class="text-lg font-medium text-white mb-2">${bill.description || bill.name}</h4>
                        <div class="text-sm text-gray-400">Valor: R$ ${bill.amount.toFixed(2)} • Vencimento: dia ${bill.dueDay}</div>
                    </div>
                    
                    <div class="space-y-3 mb-6">
                        <h5 class="text-md font-semibold text-white">Selecione o período para pagamento antecipado:</h5>
                            ...
                            <div class="period-option p-3 bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-600 transition duration-200" data-period="${period.period}" data-index="${index}">
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-400">Período: ${period.period}</div>
                                    </div>
                                    <div class="text-right">
...
// Função para processar pagamento de período específico
async function handlePeriodPayment(bill, period) {
    try {
        // Mostrar modal de confirmação bonito
        const confirmMessage = `Confirmar pagamento de R$ ${bill.amount.toFixed(2)} para o período ${period.monthName} ${period.year}?`;
...
// Função para mostrar modal de seleção de conta
function showAccountSelectionModal(bill, period) {
            ...
            <div class="bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button id="close-account-selection-modal" class="text-gray-400 hover:text-white text-2xl">×</button>
                    </div>
                    
                    <div class="mb-4">
                        <h4 class="text-lg font-medium text-white mb-2">${bill.description || bill.name}</h4>
                        <div class="text-sm text-gray-400">
                            Período: ${period.monthName} ${period.year} • Valor: R$ ${bill.amount.toFixed(2)}
                        </div>
                    </div>
    ...
    // Habilitar botão quando conta for selecionada
    ...
    confirmBtn.addEventListener('click', async () => {
        ...
        if (!selectedAccount) {
            showWarningModal('Conta selecionada não encontrada');
            return;
        }
        ...
        try {
            ...
            const result = await payFixedBill(
                ...
                period.period // período específico
            );
...
async function handleFullPayment() {
    ...
    try {
        ...
        // Fechar modal e atualizar a visualização
...
function handlePartialPaymentModal(bill) {
    ...
    // Calcular valores usando a função otimizada
    getFixedBillInfoOptimized(bill.id).then(billInfo => {
        // CORREÇÃO: Para pagamento parcial, permitir pagar próximo período se atual já foi pago
        let targetPeriod = billInfo.periodsStatus.find(p => p.isCurrentMonth);
        
        // Se período atual já foi pago completamente, usar próximo período disponível
        ...
        partialPaymentAmount.placeholder = `Máximo: R$ ${remainingAmount.toFixed(2)}`;
...
async function handlePartialPayment() {
    ...
    if (!paymentAmount || paymentAmount <= 0) {
        showWarningModal('Por favor, informe um valor válido para o pagamento');
        return;
    }
    
    const selectedAccount = accounts.find(acc => acc.id === accountId);
    if (!selectedAccount) {
        showWarningModal('Conta não encontrada');
        return;
    }
    ...
    // Calcular valor restante usando a função otimizada (mesma do modal)
    ...
    if (paymentAmount > remainingAmount) {
        showWarningModal(`Valor do pagamento (R$ ${paymentAmount.toFixed(2)}) é maior que o valor restante (R$ ${remainingAmount.toFixed(2)})`);
        return;
    }
    
    try {
        // Usar a nova função de pagamento com período correto
        const result = await payFixedBill(
            currentBillForPayment.id, 
            paymentAmount, 
            accountId, 
            false, // Pagamento parcial
            targetPeriod ? targetPeriod.period : null // Período específico se for antecipado
        );
...
function updateFinancialSummary() {
    ...
    // Atualizar também o título da página com o saldo total
...
async function handleAccountSubmit(e) {
    ...
    if (!name) {
        showWarningModal('Nome da conta é obrigatório');
        return;
    }
    
    if (isNaN(balance) || balance < 0) {
        showWarningModal('Saldo deve ser um número válido maior ou igual a zero');
        return;
    }
...
async function handleBillSubmit(e) {
    ...
    if (!name) {
        showWarningModal('Nome da conta fixa é obrigatório');
        return;
    }
    ...
    if (!startDate) {
        showWarningModal('Data inicial é obrigatória');
        return;
    }
...
function editBill(bill) {
    ...
    // Definir data inicial se disponível
...
function calculateBillStatus(bill) {
    ...
    // Verificar se já foi paga (baseado em transações)
    const billTransactions = transactions.filter(t => 
        t.billId === bill.id && 
        t.type === 'expense' &&
        (t.isPartialPayment || t.isBillPayment)
    ...
    if (buttons && Array.isArray(buttons)) {
        ...
        buttons.forEach(button => {
            ...
            buttonElement.addEventListener('click', () => {
                if (!button.disabled) {
                    // Ocultar todos os botões da mensagem
...
// Função de debug para investigar auto-expansão
async function debugAutoExpansion(billId) {
    try {
        console.log('🔍 === DEBUG AUTO-EXPANSÃO ===');
        ...
        console.log('📊 Dados da conta fixa:', {
        ...
        console.log('📋 Períodos calculados:', periods.map(p => ({
            period: p.period,
            monthName: p.monthName,
            year: p.year
        })));
        
        // Verificar pagamentos de cada período
        const paidPeriods = [];
        const unpaidPeriods = [];
        ...
        console.log('✅ Períodos pagos:', paidPeriods);
        console.log('⏳ Períodos não pagos:', unpaidPeriods);
    ...
    try {
        ...
        for (const bill of fixedBills) {
            try {
                ...
                console.warn(`Erro ao verificar expansão da conta ${bill.description}:`, error);
            }
        }
        
        // Se houve expansões, mostrar notificação
        if (expandedCount > 0) {
            const message = expandedCount === 1 
                ? `✅ ${expansionResults[0].billName}: ${expansionResults[0].message}`
                : `✅ ${expandedCount} contas fixas foram expandidas automaticamente!`;
        ...
        console.error('Erro ao verificar expansão de contas fixas:', error);
        throw error;
    }
}

// Função para mostrar histórico de pagamentos
async function showPaymentHistory(billId) {
    try {
        // Usar função otimizada para obter histórico
        const paymentHistory = await getFixedBillPaymentHistoryOptimized(billId, 12);
        
        // Buscar dados básicos da conta fixa
        const billRef = doc(db, 'users', auth.currentUser.uid, 'recurring_bills', billId);
        const billDoc = await getDoc(billRef);
        ...
        // Renderizar histórico otimizado
            ...
            <div class="bg-gray-700 rounded-lg p-4 mb-3">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-sm ${period.totalPaid >= billData.amount ? 'text-green-400' : 'text-yellow-400'}">
                        ${period.totalPaid >= billData.amount ? '✅ Pago' : '⚠️ Parcial'}
                    </span>
                </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-600">
                        ...
                        ${period.payments.map(payment => `
                            <div class="text-xs text-gray-300 mb-1">
                                • R$ ${payment.amount.toFixed(2)} - ${new Date(payment.date?.toDate ? payment.date.toDate() : payment.date).toLocaleDateString('pt-BR')}
                            </div>
                        `).join('')}
                    </div>
                ` : ''}
        ...
        content.innerHTML = `
            <div class="mb-6">
                ...
                <div class="text-sm text-gray-400">
                    Valor: R$ ${billData.amount.toFixed(2)} • Vencimento: dia ${billData.dueDay}
                </div>
            </div>
            
            <div class="space-y-3">
                <h5 class="text-md font-semibold text-white mb-3">📊 Histórico de Pagamentos</h5>
        ...
        showErrorModal('Erro ao carregar histórico de pagamentos');
    }
}
...
// Função para atualizar interface após estorno de transação
async function updateInterfaceAfterRefund(transactionId) {
    try {
        console.log('🔄 Atualizando interface após estorno...');
        
        // Remover transação do cache local
        ...
        // Remover das transações recentes também
        transactions = transactions.filter(t => t.id !== transactionId);
        
        // Atualizar interface das transações recentes
        renderRecentTransactions();
        
        // Se o modal de transações estiver aberto, atualizar também
        if (!transactionsModal.classList.contains('hidden')) {
            renderTransactionsOptimized();
        }
        
        // Se o modal de histórico de pagamentos estiver aberto, atualizar também
        const paymentHistoryModal = document.getElementById('payment-history-modal');
        ...
        console.log('✅ Interface atualizada após estorno');
        
    } catch (error) {
        console.error('Erro ao atualizar interface após estorno:', error);
    }
}

// Função para estornar transação (deve ser global para o onclick)
window.handleRefund = async function(transactionId) {
    try {
        // Verificar se a transação ainda existe no cache local
        const transactionExists = allLoadedTransactions.some(t => t.id === transactionId) || 
                                 transactions.some(t => t.id === transactionId);
        
        if (!transactionExists) {
            showWarningModal('Esta transação já foi estornada ou não existe mais.');
            return;
        }
        ...
        showConfirmModal(
            'Confirmar Estorno',
            'Tem certeza que deseja estornar esta transação? Esta ação irá reverter o saldo da conta e remover a transação do histórico.',
            async () => {
                try {
                    const refundResult = await refundTransaction(transactionId);
                    showSuccessModal(refundResult.message);
                    
                    // Atualizar interface imediatamente após estorno
                    await updateInterfaceAfterRefund(transactionId);
                    
                } catch (error) {
                    showErrorModal(`Erro ao estornar transação: ${error.message}`);
                }
            }
        );
    } catch (error) {
...
// Função global para carregar mais transações
...
// Função para estornar transação (versão interna)
...
// Teste para verificar se tudo está funcionando
console.log('🔧 Testando sistema de estorno...');

// Verificar se a função está disponível globalmente
if (typeof window.handleRefund === 'function') {
} else {
    console.log('❌ Função handleRefund NÃO está disponível globalmente');
}

// Verificar se a função refundTransaction está sendo importada
if (typeof refundTransaction === 'function') {
} else {
    console.log('❌ Função refundTransaction NÃO está disponível');
}

// Verificar se a função showConfirmModal está disponível
if (typeof showConfirmModal === 'function') {
} else {
    console.log('❌ Função showConfirmModal NÃO está disponível');
}

console.log('🎉 Sistema de estorno testado com sucesso!');
